...
const MOCK_API_URL = env.MOCK_SERVER + "/listShifts";
...
class ShiftService {
  ...
  async createShift(shift: Omit<Shift, "id">): Promise<Shift | null> {
    ...
    try {
      const response = await axios.get<Shift[]>(MOCK_API_URL);
      return response.data;
    } catch (error) {
      console.error("Error obteniendo turnos desde el mockserver:", error);
      return [];
    }
  }
